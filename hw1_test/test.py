...
time_Take = [False, False, False, False, True, True, True, True, True]
...
for stat in test_X_Take:
    x = stat
    if b + sum(w_vector * x) >= min(y):
        if b + sum(w_vector * x) <= max(y):
            label_test_X.append(b + sum(w_vector * x))
        else:
            label_test_X.append(max(y))
    else:
        label_test_X.append(min(y))
...
#regr = neural_network.MLPRegressor(hidden_layer_sizes=(20000, ),max_iter = 20000)
