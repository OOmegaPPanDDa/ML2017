...
checkpoint = ModelCheckpoint('dnn_model.h5',
                             verbose=1,
                             save_best_only=True,
...
model.save('dnn_complete_model.h5')
del model


model = load_model('dnn_complete_model.h5')
...
f = open('dnn_prediction.csv', 'w', encoding = 'big5', newline='')
w = csv.writer(f)
w.writerows(result)
f.close()
